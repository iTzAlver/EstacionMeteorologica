...
{	
	...
	LPC_GPDMACH0->DMACCDestAddr = (uint32_t) &(LPC_DAC->DACR) + 1;
...
void __configuraAudio__()
{
	...
	LLI0.destination = (uint32_t) &(LPC_DAC->DACR) + 1;
	LLI0.next        = (uint32_t) &LLI0;
	LLI0.control     = 1<<26 | 0<<21 | 0<<18 | MUESTRAS_AUDIO; 	//	Incremento origen, MUESTRAS_AUDIO muestras, 8 bits todo.
	...
	LPC_GPDMACH0->DMACCSrcAddr  = (uint32_t) AUDIO;			//	Empieza en AUDIO.
	LPC_GPDMACH0->DMACCDestAddr = (uint32_t) &(LPC_DAC->DACR) + 1;	//	Ve a DACR + 2.
	LPC_GPDMACH0->DMACCLLI      = (uint32_t) &LLI0; // linked lists for ch0
